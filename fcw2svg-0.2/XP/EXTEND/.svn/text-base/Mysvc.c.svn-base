...
///////////// GetInt - Read int from string //////////////////////////

static char GetInt(char **Pek, int *Num)
{
	char Negate=1;
...
///////////// GetReal - Read real from string ////////////////////////

static char GetReal(char **Pek, float *Num, int *DecimalCount)
{
	char Negate=FALSE;
	char Found=FALSE;
...
void FloatToStr(float *pValue, char *pStr, BYTE ndecp)
{
	FORMSTPKT(lpszFloat, "!01", 1)
    {(DWORD *)pValue, FT_Real4, FJ_Var, 5, ndecp}, 
	FORMSTEND

	FormSt(&lpszFloat, pStr);
}

void FloatFromStr(float *pValue, char *pStr, BYTE ignore)
{
	GetReal(&pStr, pValue, NULL);
}
...
void _stdcall DevUseColor(void);
void _stdcall DevUseColorE(void);
...
void XPCALL _DevUseColor(int Color)
{
	__asm mov eax, Color;
	DevUseColor();
}

void XPCALL _DevUseColorE(int Color)
{
	__asm mov eax, Color;
	DevUseColorE();
}
